...
class MyComputer {
  ...
  function loadFile( $file_path ) {
    $input_as_string = file_get_contents( $file_path );
    if( false !== $input_as_string ) {
      $nrs = explode( ',', $input_as_string);

      // fix program 
      $nrs[1] = 12;
      $nrs[2] = 2; 
      $result = $this->parseInput( $nrs ); 
    }
    echo $result;   
  }
  ...
  function parseInput( array $input ) {
    $this->numbers = $input;  
    $nr_codes = sizeof( $this->numbers );  
    for( $i=0; $i < $nr_codes; $i = $i+4) {
      $opcode = $this->numbers[$i];
      switch( $opcode){
      case 1:
        $this->processNumbers( $i, 'add' ); 
        break;
      case 2:
        $this->processNumbers( $i, 'multiply' );
       break; 
      case 99: 
        break;   
      }
  ...
  /**
   * Use the opcode position to determine the next two values 
   * to be added or multiplied  and the output position to which the result
   * should be written.
   */
  function processNumbers( $opcode_pos, $operation ) {
    $first_value_position  = $this->getValue ( $opcode_pos + 1 );
    $second_value_position = $this->getValue ( $opcode_pos + 2 ); 
    $output_value_position = $this->getValue ( $opcode_pos + 3 );   
    
    $first_val = $this->getValue( $first_value_position ); 
    $second_val = $this->getValue( $second_value_position ); 
    ...
    $this->setValue( $output_value_position, $sum );
  }
...
function testMyComputer() {
  ...
  foreach( $test_input as $input => $expected_output ) {
    $numbers = explode( ',', $input);
    $computed_result = $computer->parseInput( $numbers );
